...
```bash
# Запуск инфраструктуры (Keycloak, Postgres, Redis, BFF, Frontend)
docker-compose up -d

# Точки доступа:
# - Frontend (React):     http://localhost:3000
# - BFF / API Gateway:    http://localhost:3001
# - Keycloak Admin Panel: http://localhost:8080 (admin / admin)
# - Redis Token Vault:    localhost:6379
...
Данная архитектура обеспечивает безопасное управление учётными данными в системе BionicPRO с соблюдением следующих требований:

1. **Унификация доступа** — централизованная аутентификация через IdP Broker (Keycloak) с поддержкой внешних удостоверяющих служб разных стран
2. **Безопасность токенов** — фронтенд НЕ получает токены IdP, работает только через session cookies
3. **Локальное хранение данных** — персональные и медицинские данные хранятся в стране представительства

## Компоненты архитектуры

### 1. Identity Federation Layer

**IdP Broker (Keycloak)** — единая точка входа для аутентификации из разных стран

- Подключается к внешним IdP через OIDC/SAML 2.0
- Маппит внешние идентификаторы на внутренние учётные записи
- Выпускает JWT токены для внутренних сервисов

**Внешние IdP:**
- Россия: Госуслуги / корпоративный IdP (OIDC/SAML)
- Германия: eID / NPA (SAML 2.0)
- Другие страны: локальные удостоверяющие службы

### 2. BFF / API Gateway

**Backend-for-Frontend** — проксирует запросы фронтенда к API

- Управляет сессиями пользователей
- Хранит токены в Token Vault (фронтенд не имеет к ним доступа)
- Проксирует запросы к API с валидными токенами
- Автоматически обновляет access tokens

### 3. Token Vault

**Redis (зашифрованное хранилище)** — безопасное хранение refresh токенов

- Токены шифруются AES-256-GCM
- Привязка к session_id
- Автоматическое истечение срока жизни

### 4. PKCE Flow

**Proof Key for Code Exchange (RFC 7636)** — безопасная аутентификация для публичных клиентов

```
Frontend → BFF → Keycloak → External IdP
   ↑                              ↓
   └──── session cookie ──────────┘
```

1. Frontend инициирует login
2. BFF генерирует code_verifier + code_challenge (SHA-256)
3. BFF редиректит на Keycloak с code_challenge
4. Keycloak аутентифицирует пользователя через внешний IdP
5. Keycloak возвращает authorization code
6. BFF обменивает code + code_verifier на tokens
7. BFF сохраняет tokens в Token Vault
8. Frontend получает только session cookie (HttpOnly, Secure)
...
│   │   ├── services/
│   │   │   └── token-vault.ts     # Безопасное хранение токенов
│   │   └── utils/
│   │       └── pkce.ts            # PKCE утилиты
│   ├── Dockerfile
│   ├── package.json
│   └── tsconfig.json
...
│       ├── services/
│       │   └── auth.ts            # Auth service (работа с BFF)
│       └── utils/
│           └── pkce.ts            # PKCE утилиты (клиент)
├── keycloak/
│   └── realm-export.json          # Конфигурация Keycloak с PKCE
├── docker-compose.yaml             # Оркестрация сервисов
└── BionicPRO_C4_model_security.drawio.xml  # Диаграмма C4
```
...
| Атака | Мера защиты |
|-------|-------------|
| XSS | HttpOnly cookies, токены не доступны JS |
| CSRF | SameSite=strict cookies, state parameter |
| Token interception | Токены хранятся только на сервере |
| Authorization code interception | PKCE (S256) |
| Replay attacks | Nonce, state parameter |
| Session hijacking | Secure cookies, короткоживущие сессии |

### Принципы

1. **Токены IdP никогда не покидают сервер** — фронтенд получает только session cookie
2. **Refresh tokens зашифрованы** — AES-256-GCM в Redis
3. **PKCE обязателен** — S256 для всех клиентов
4. **Минимальные привилегии** — RBAC для доступа к ресурсам
...
Обновлённая диаграмма C4 находится в файле `BionicPRO_C4_model_security.drawio.xml`.

Новые компоненты:
- **IdP Broker (Keycloak)** — федерация идентичности
- **BFF / API Gateway** — проксирование запросов
- **Token Vault (Redis)** — хранение refresh токенов
- **External IdP (Russia/Country X)** — внешние удостоверяющие службы